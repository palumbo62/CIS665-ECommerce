...
//*************************************************************
// FUNCTION:    bogCheckEmailExists
//
// PURPOSE:     Used to check if an email address already
// exists within the database.  Returns true if emails address
// exists false otherwise.
// 
//*************************************************************
function bogCheckEmailExists($email)
{
    // the SQL query to be executed on the database

    $query = <<<STR
exec spBogCheckEmailExists '$email';
STR;
   ...
   $res = count($user) == 1 ? true : false;
   echo "CHECK EMAIL='$res'<br>";
   return $res;
}
...
function bogAddPropProf($proptype, $title, $address, $city, $state, 
            ...
            $sqft, $guests, $pic, $imageName, $description)
{
    // the SQL query to be executed on the database
...
//
//*************************************************************
function bogAddReservProf($propId, $userId, 
                            $checkInDate, $checkOutDate, $guestCnt)
{
    // Make sure all dates are in YYYY/MM/DD format for the DB
    $cid = date("m/d/Y", strtotime($checkInDate));
    $cod = date("m/d/Y", strtotime($checkOutDate));
    //var_dump($checkInDate, $checkOutDate); die();   

    // the SQL query to be executed on the database

    $query = <<<STR
exec spBogAddReservProf $propId, $userId, '$cid', '$cod', 1500, $guestCnt;
STR;
   //var_dump($query, $checkInDate, $checkOutDate); die();   
   // execute the query and return the result
   return executeQuery($query);
}

//*************************************************************
// FUNCTION:    bogCheckReservExists
//
// PURPOSE:     Used to determine if a reservation exists 
// overlapping with the specified date range
//
//*************************************************************
function bogCheckReservExists($propId, $checkinDate, $checkoutDate)
{
    // Make sure all dates are in YYYY/MM/DD format for the DB
    $cid = date("m/d/Y", strtotime($checkinDate));
    $cod = date("m/d/Y", strtotime($checkoutDate));
    ...
    $query = <<<STR
exec spBogCheckReservExists $propId, '$cid', '$cod'
STR;
   //var_dump($query); die();
   // execute the query and return the result
   return executeQuery($query);
...
//*************************************************************
// FUNCTION:    bogGetReservProfByPropId
//
// PURPOSE:     Used to retrieve any reservations made by the
// specified property id.
//
//*************************************************************
function bogGetReservProfsByPropId($propId)
{
    // the SQL query to be executed on the database

    $query = <<<STR
exec spBogGetReservProfsByPropId $propId
STR;
...
{
    $query = <<<STR
Select p.[PropertyIdPK], p.[PropertyTypeIdFK], 
    Address, City, State, Zipcode,
    DailyPrice, NumBedrooms, NumBathrooms, Sqft, 
    GuestCnt, ImageName, Description
...
Select p.[PropertyIdPK], p.[PropertyTypeIdFK],
    Address, City, State, Zipcode,
    DailyPrice, NumBedrooms, NumBathrooms, SqFt, 
    GuestCnt,  pt.[PropertyTypeName], ImageName, Description,
    PropertyTitle        
From PropertyT p
    Inner Join PropertyTypeT pt
        On p.[PropertyTypeIdFK] = pt.[PropertyTypeIdPK]
...
//*************************************************************
// FUNCTION:    bogAddComments
//
// PURPOSE:     Used to add a new comments to the BOG system.
//
//*************************************************************
function bogAddCommentsProf($userId, $propId, $rating,
            $mmyyVisit, $comments)
{
    // the SQL query to be executed on the database
    $query = <<<STR
exec spBogAddCommentsProfByProfIdUserId $propId, $userId, $rating,
            '$mmyyVisit', '$comments';
